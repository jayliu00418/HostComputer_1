...
ALP_ERRORS = {
    1001: "ALP设备未找到或ALP设备序号无效",
    1002: "ALP设备不在理想状态",
    1003: "ALP设备未找到或ALP设备序号无效",
    1004: "ALP设备已连接",
    1005: "参数无效",
    1006: "用户数据错误",
    1007: "ALP设备内存不可用（已满？）",
...
class ALP4(object):
    ...
    def __init__(self, version="4.3", libDir=r"D:/pythonProjectDAHENG"):
        self.IsDMDdevalloc = False
    ...
    def _checkError(self, returnValue, errorString, warning=False):
        if not (returnValue == ALP_OK):
            if not warning:
                ...
                #print(ALPError(returnValue))
                return ALPError(returnValue)
            else:
    ...
    def Initialize(self, DeviceNum=None):
        ...
        errorvalue = self.nSizeX = nSizeX.value
        ...
        #return "已初始化DMD，分辨率为： "+ str(self.nSizeX)+ " x "+ str(self.nSizeY)+ "."
        #resolution = self.nSizeX,self.nSizeY
        #npdata = np.array(resolution)
        if errorvalue is not None:
            return errorvalue
        else:
            self.IsDMDdevalloc = True
            return self.nSizeX, self.nSizeY
    ...
    def ProjHalt(self):
        ...
        Usage: Halt()
        """
        self._checkError(self._ALPLib.AlpProjHalt(self.ALP_ID), "Cannot stop Proj.")
    ...
    def Free(self):
        ...
        """
        errorvalue = None
        errorvalue = self._checkError(self._ALPLib.AlpDevFree(self.ALP_ID), "Cannot free device.")
        del self._ALPLib
        self.IsDMDdevalloc = False
        if errorvalue is not None:
            return errorvalue
